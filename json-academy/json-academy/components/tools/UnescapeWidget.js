...
export default function UnescapeWidget() {
  ...
  return (
    <ToolShell title="JSON Unescape" activeSlug="json-unescape" theme={et} setTheme={setTheme} right={output && <ToolbarBtn theme={et} onClick={handleCopy}><Icon name={copied ? "check-circle" : "copy"} className="w-3.5 h-3.5" />{copied ? "Copied!" : "Copy"}</ToolbarBtn>}>
      <div className="grid min-h-[640px] grid-cols-1 gap-4 xl:grid-cols-2">
        <div className="flex min-h-0 flex-col overflow-hidden rounded-[24px] border" style={{ backgroundColor: et.panelBg, borderColor: et.panelBorder }}>
          <div className="flex h-11 shrink-0 items-center justify-between px-4" style={{ borderBottom: `1px solid ${et.divider}` }}>
            <span className="text-xs font-semibold uppercase tracking-[0.2em]" style={{ color: et.labelFg }}>Escaped text</span>
          ...
          <div className="min-h-0 flex-1 p-3">
            <div className="relative h-full overflow-hidden rounded-lg" style={{ border: `1px solid ${et.panelBorder}`, backgroundColor: et.shell }}>
              <textarea value={input} onChange={(e) => setInput(e.target.value)} spellCheck={false} placeholder="Paste escaped text to unescape…" className={ta}
        ...
        <div className="flex min-h-0 flex-col overflow-hidden rounded-[24px] border" style={{ backgroundColor: et.panelBg, borderColor: et.panelBorder }}>
          <div className="flex h-11 shrink-0 items-center px-4" style={{ borderBottom: `1px solid ${et.divider}` }}>
            <span className="text-xs font-semibold uppercase tracking-[0.2em]" style={{ color: et.labelFg }}>Unescaped output</span>
          </div>
          <div className="min-h-0 flex-1 p-3">
            <div className="relative h-full overflow-hidden rounded-lg" style={{ border: `1px solid ${et.panelBorder}`, backgroundColor: et.shell }}>
              <textarea readOnly value={output} spellCheck={false} placeholder="Unescaped text appears here…" className={ta}
                style={{ lineHeight: "20px", color: et.editorFg, "--tw-ring-color": et.accent }} />
            </div>
          </div>
          <div className="shrink-0 px-4 py-2" style={{ borderTop: `1px solid ${et.divider}`, backgroundColor: et.footerBg }}>
            <p className="text-xs" style={{ color: et.footerFg }}>{output ? `${output.length.toLocaleString()} chars · processed locally` : "Paste escaped text in the input panel"}</p>
          </div>
        </div>
      </div>
    </ToolShell>
