...
function AnnotatedEditor({ value, onChange, errorLine, theme }) {
  ...
  return (
    <div className="relative h-full min-h-0 overflow-hidden rounded-lg border font-mono text-[13px]"
...
export default function ValidatorWidget() {
  ...
  return (
    <div className="fixed inset-0 flex overflow-hidden" style={{ backgroundColor: et.wrapperBg }}>
      <FormatterToolPanel theme={et} activeSlug="json-validator" />

      <div className="flex min-w-0 flex-1 flex-col">
        <TopBar theme={et} title="JSON Validator" right={
          !isEmpty && (
        ...
        <div className="flex min-h-0 flex-1 flex-col">
          <div className="flex h-9 shrink-0 items-center justify-between px-4" style={{ borderBottom: `1px solid ${et.shellBorder}`, color: et.labelFg }}>
            <span className="text-xs font-semibold">Paste JSON to validate</span>
          ...
          {!isEmpty && (
            <div className={`shrink-0 px-4 py-3 ${valid ? "" : ""}`} style={valid ? { borderTop: `1px solid ${et.divider}`, backgroundColor: et.footerBg } : { borderTop: `1px solid ${et.divider}`, backgroundColor: et.footerBg }}>
