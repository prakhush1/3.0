    ...
    <main className="overflow-x-hidden">
      ...
      <section className="mx-auto max-w-6xl px-6 pb-10 pt-6">
        ...
        <h1 className="mt-3 text-5xl font-extrabold tracking-tight sm:text-6xl">
          JSON <span className="text-violet-400">Toolbox</span>
        </h1>
        ...
        <div className="mt-10 flex flex-wrap items-center gap-3">
          <div className="flex min-w-[260px] flex-1 items-center gap-2 rounded-lg border border-gray-200 bg-white px-4 py-3">
            <Icon name="search" className="w-4 h-4 text-gray-400" />
            <input
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              placeholder="Search tools..."
              className="w-full bg-transparent text-sm text-[#171717] placeholder-gray-400 outline-none"
            />
          </div>
              ...
              <button
                ...
                className={
                  ...
                  (filter === f
                    ...
                    : "border border-gray-200 bg-white text-[#171717] hover:bg-gray-50")
                }
              >
                {f}
              </button>
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-3">
          {filtered.map((t) => (
            <a key={t.slug} href={`/tools/${t.slug}`} className="group rounded-2xl border border-gray-200 bg-white p-6 transition hover:border-violet-200 hover:shadow-md">
              ...
              <p className="mt-0.5 text-sm font-medium text-violet-500">{t.sub}</p>
