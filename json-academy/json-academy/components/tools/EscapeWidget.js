      ...
      <div className="flex h-12 shrink-0 items-center justify-between border-b border-white/10 px-4">
        <div className="flex items-center gap-5">
          ...
          <nav className="flex items-center gap-1">
            ...
            <span className="ml-1 text-xs font-semibold text-violet-400">JSON Escape</span>
          </nav>
        </div>
      ...
      <div className="flex min-h-0 flex-1 divide-x divide-white/10">
        <div className="flex min-h-0 flex-1 flex-col">
          <div className="flex h-9 shrink-0 items-center justify-between border-b border-white/10 px-4">
            <span className="text-xs font-semibold text-gray-400">Raw text</span>
          ...
          <div className="min-h-0 flex-1 p-3">
            <div className="relative h-full overflow-hidden rounded-lg border border-white/8 bg-[#0d1117]">
              <textarea value={input} onChange={(e) => setInput(e.target.value)} spellCheck={false} placeholder="Paste raw text to escape…" className={ta} style={{ lineHeight: "20px" }} />
            </div>
          </div>
        </div>
        <div className="flex min-h-0 flex-1 flex-col">
          <div className="flex h-9 shrink-0 items-center border-b border-white/10 px-4">
            <span className="text-xs font-semibold text-gray-400">Escaped output</span>
          </div>
          <div className="min-h-0 flex-1 p-3">
            <div className="relative h-full overflow-hidden rounded-lg border border-white/8 bg-[#0d1117]">
              <textarea readOnly value={output} spellCheck={false} placeholder="Escaped text appears here…" className={ta} style={{ lineHeight: "20px" }} />
            </div>
          </div>
          <div className="shrink-0 border-t border-white/10 px-4 py-2">
            <p className="text-xs text-gray-600">{output ? `${output.length.toLocaleString()} chars · processed locally` : "Paste text in the input panel"}</p>
          </div>
        </div>
      </div>
    </div>
